...
public class command implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand( CommandSender sender,  Command command,  String label,  String[] args) {
        ...
        else if (args.length==1)
        {
            ...
            {
                sender.sendMessage(Config.plugin+MsgUtils.msg("- &e&l指令帮助"));
                sender.sendMessage(MsgUtils.msg("&f &f - &3/lshop admin reload &f重载插件"));
                sender.sendMessage(MsgUtils.msg("&f &f - &3/lshop admin random &f强制随机今日商店"));
                sender.sendMessage(MsgUtils.msg("&f &f - &3/lshop admin add 稀有等级 出售/收购 vault/points 最低价格;最高价格 限售/购数量(-1为无限) &f将手上的物品增加到随机池商品"));
                sender.sendMessage(MsgUtils.msg("&f &f - &3/lshop admin remove (common/rare/epic/legendary) &f打开商品删除GUI"));
            }
        ...
        else if (args.length==3)
        {
            ...
            if (args[0].equalsIgnoreCase("admin") && args[1].equalsIgnoreCase("remove") && sender.isOp()) {
                if (sender instanceof Player) {
                    ...
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f物品稀有等级输入错误 &7&o(common/rare/epic/legendary)"));
                        return true;
                    }
        ...
        else if (args.length==7)
        {
            if (args[0].equalsIgnoreCase("admin") && args[1].equalsIgnoreCase("add") && sender.isOp())
            {
                if (sender instanceof Player)
                {
                    ...
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f请手持需要上架的物品"));
                        return true;
                    }
                    if (!args[2].equals("common") && !args[2].equals("rare") && !args[2].equals("epic") && !args[2].equals("legendary"))
                    {
                    ...
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f商品类型出错 &7&o(出售/收购)"));
                        return true;
                    }
                    if (!args[4].equals("vault") && !args[4].equals("points"))
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f商品货币类型出错 &7&o(vault/points)"));
                        return true;
                    }
                    if (!args[5].contains(";"))
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f价格格式为 &a最低价格;最高价格 &7例如: 5.0;10.0 或者希望价格固定 则 5.0;5.0"));
                        return true;
                    }
                    String[] prices=args[5].split(";");
                    if (!isNumeric(prices[0]) || !isNumeric(prices[1]))
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f请输入正确的价格"));
                        return true;
                    }
                    if (!isMath(args[6].replace("-","")))
                    {
                        sender.sendMessage(Config.plugin+MsgUtils.msg("&f请输入正确的限售/限购数量"));
                        return true;
                    }
